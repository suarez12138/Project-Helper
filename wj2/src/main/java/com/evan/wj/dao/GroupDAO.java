...
public interface GroupDAO extends JpaRepository<User,Integer> {
    ...
    @Query("select new com.evan.wj.bean.MyGroup_getNameGender(p.id,p.stu_id, p.name, p.gender) from PersonGroup pg join People p on pg.person = p.id where pg.gro = ?1")
